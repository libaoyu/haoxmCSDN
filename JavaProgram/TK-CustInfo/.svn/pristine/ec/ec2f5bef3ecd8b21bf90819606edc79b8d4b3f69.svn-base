...
<title></title>
<link rel="stylesheet" href="../static/question/css/queryPage.css">
...
<script type="text/javascript">
	...
	function saveDataForList() {
		var varList="${varList}";
		if(varList=='' ||varList== null||varList.length<0){
			alert("没有数据不允许提交！");
			return;
		}
...
<body >
		...
		<div class="survey_title" id="main_Title">
			<div class="inner">
				<div class="title_content">
					<!-- <p>2017年泰康之家养老社区护理区<br>客户满意度调研问卷</p> -->
				...
				<div class="main_border">
					<div class="mian_inner_content">
						...
						<div style="margin: 3% 3%; line-height: 1.5; text-align: justify">
							...
							<!-- 尊敬的客户，您好！为持续改进和提升我们的服务品质， 泰康之家总部质量和精益管理部每年定期向客户发放满意度调查问卷， 以倾听客户的声音。恳请您抽出宝贵时间，填写这份问卷。 我们会对您所填写的内容予以保密，感谢您的合作与支持！ 如果您希望做更详细深入的交流，可致电泰康之家客户服务热线：
							<b>40060-95522</b>
						...
						<form action="#" method="post" name="Form"
							id="questionForm" >
							<input id="batchId" name="batchId" type="hidden" value="${batch }" /> <input
								name="questionnaireId" type="hidden" value="${questionnaireId }" />
							<input name="level" type="hidden" value="${pd.level }" /> <input
								name="column1" type="hidden" value="" />
							<c:forEach items="${ varList}" var="entity">
								<c:if test="${entity.classification=='L0' }">
								<c:if test="${entity.types=='T16' }">
									<div class="hr_div">
										<hr class="f1 user_hr1">
										<p class="f1 user_p1">推荐意向</p>
										<hr class="f1 user_hr1">
										<div class="c1"></div>
									</div>
								</c:if>
									<div id="${entity.questionId }frame" class="user_div01">
									<c:if test="${(entity.types=='T1' || entity.types=='T2') and entity.types!='T16' }">
										<span class="user_span03 f1" id="span_${entity.questionId }">
											${entity.title } </span>
									</c:if>
									<c:if test="${entity.types!='T1' and entity.types !='T2'  and entity.types!='T16'}">
	...
	<c:if test="${ entity.types=='T8'}">
		...
		<c:if test="${ entity.types=='T16'}">
			...
			<c:if test="${entity.scaleType=='ST11' }">
				<div data-toggle="modal" data-target="#id_${entity.questionId}" onclick="choseNumFor('${entity.questionId}','${entity.types }');"
					class="user_div02">
					<p class="f1" id="${entity.questionId}-id"  
						name="${entity.questionId}">请选择</p>
						<input name="${entity.questionId }_${entity.types }"
				id="${entity.questionId }_${entity.types }"
				type="hidden">
					<p class="f2">
				...
				<div id="id_${entity.questionId}" class="modal fade"
					style="display: none;">
					<!--最底层的半透明遮罩-->
			...
			<c:if test="${entity.scaleType=='ST10' }">
				<table class="table-bordered" >
					<tr class="user_tr">
						<c:forEach items="${entity.questionDataMap }" var="myMap" varStatus="stt">
							<td><label class="f1"> 
								<input class="user_in03 f1" name="${entity.questionId }_${entity.types }"
									type="radio" value='${myMap.key}' id="${entity.questionId}_checkId${stt.index+1}"
									onclick="setRadio('${entity.questionId}_checkId${stt.index+1}',this)">
									<a onclick="clickRadioForA('${entity.questionId}_checkId${stt.index+1}');">
			...
			<c:if test="${entity.mustFlag==1 }">
					<div class="c1">
						<a class="hide_p" id="mustFlag_${entity.questionId}"
							style="display: none">这道题必须回答哦。</a>
					</div>
			</c:if>
			</div>
	...
	<div class="containerTitle">
		<p>所有标记为“*”的题目均为必答题，选项含义如下:</p>
		<table class="table-bordered table-responsive">
			<tbody>
				<tr class="user_td text-center">
					<td width="25%;">非常不满意</td>
					<td width="19%;">不满意</td>
					<td width="18%;">一般</td>
					<td width="19%;">满意</td>
					<td width="19%;">非常满意</td>
				...
				<tr class="user_td text-center">
					<td><label class=""> <span>1分</span>
					</label></td>
					<td><label class=""> <span>2分</span>
					</label></td>
					</label></td>
					<td><label class=""> <span>3分</span>
					</label></td>
					
					<td><label class=""> <span>4分</span>
					</label></td>
					<td><label class=""> <span>5分</span></label></td>
				</tr>
			</tbody>
		</table>
		<p style="color: #ff6405">提示：未体验/不清楚的项目可以选择“不适用”</p>
	</div>
	<!--section end-->
	<!--main-->
	<div class="main">
		...
		<c:forEach items="${ varList}" var="entity">
			<c:if test="${entity.classification =='L1'}">
				<div class="containerTitle">
					<b class="ti_b1"> ${entity.questionCode}.${entity.title } <c:if
				...
				<c:forEach items="${entity.SUBQUESTIONS }" var="childEntity"
					varStatus="st">
					<div id="${childEntity.questionId }frame"  <c:if test="${childEntity.mustFlag==1 }"> class="container" </c:if>  <c:if test="${childEntity.mustFlag!=1 }"> class="containerTitle" </c:if>>
						<a name="${childEntity.questionId }"></a>
						<c:if test="${!empty childEntity.title   }">
							<p class="ti_p1">

								${entity.questionCode}${st.index+1 }.${childEntity.title }
...
$(document).ready(function(){
	...
	function checkedCookie(){
		...
		//alert(typeof(cookieVlaue) !=  'undefined');
		//alert(cookieVlaueForCheck);
