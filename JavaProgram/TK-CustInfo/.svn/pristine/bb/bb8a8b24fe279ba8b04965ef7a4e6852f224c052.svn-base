...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="questions/list.do?id=z31" method="post" name="Form" id="Form">
			<table style="width: 100%;">
				<tr>
					<td style="width: 18%;">
						<span class="input-icon">
							<input autocomplete="off" id="title" type="text" name="title" value="${pd.title }" placeholder="输入问题题目查询" />
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 30%;">${var.questionnaireTitle}</td>
								<td style="width: 10%;">
									<c:if test="${var.classification == 'L2'}">
										${var.classify}
									</c:if>
								</td>
								<td style="width: 8%;">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(pageId){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>questions/goAdd.do?questionnaireId='+pageId;
			 diag.Width = 1050;
			 diag.Height = 755;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>questions/goEdit.do?questionId='+Id;
			 diag.Width = 850;
			 diag.Height = 755;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
