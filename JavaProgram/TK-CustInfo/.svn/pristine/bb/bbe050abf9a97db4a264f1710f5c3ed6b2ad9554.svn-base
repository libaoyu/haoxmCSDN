...
<html lang="en">
	...
	function changeLeve(obj){
		...
		oneLevelChangeByQuestionnaire(questionnaireId,$(obj).val());
	...
	function oneLevelChangeByQuestionnaire(questionnaireIdVal,levelVal){
		...
		$.ajax({
			type: "POST",
			url: 'questionnaireTemp/questionnaireTemp_change',
...
<body class="g_wrapper page_edit" onload="fun_loadData();">
	<form action="questionnaireTemp/${msg}.do" name="Form" id="Form" method="post">
	<input type="hidden" name="questionId" id="questionId" onchange="" value="${question.questionId}"/>
	<input type="hidden" name="respondentsParentId" id="respondentsParentId" onchange="" value="${question.respondentsParentId}"/>
