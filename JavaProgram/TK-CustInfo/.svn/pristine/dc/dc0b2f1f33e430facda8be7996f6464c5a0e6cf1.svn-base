		...
		<div id="sidebar" class="menu-min">
				...
				<div id="sidebar-shortcuts">
					...
					<div id="sidebar-shortcuts-large">
						...
						<button class="btn btn-small btn-info" title="设计案例"><i class="icon-eye-open"></i></button>
				...
				<li id="lm${menu.MENU_ID }">
					  ...
					  <ul class="submenu">
							<c:forEach items="${menu.subMenu}" var="sub">
								<c:choose>
									<c:when test="${sub.hasMenu}">
									<li id="z${sub.MENU_ID }">
										<a style="cursor:pointer;" target="mainFrame"  onclick="siMenu('z${sub.MENU_ID }','lm${menu.MENU_ID }','${sub.MENU_NAME }','${sub.MENU_URL }')"><i class="icon-double-angle-right"></i>${sub.MENU_NAME }</a>
									</li>
									</c:when>
									<%-- <c:otherwise>
