...
<body class="no-skin">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-sm btn-success" onclick="add('${linkageId}');">新增</a>
									</c:if>
									<c:if test="${null != pd.id && pd.id != ''}">
									<a class="btn btn-sm btn-success" onclick="goSondict('${pd.pId}');">返回</a>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		//去此ID下子列表
		function goSondict(linkageId){
			top.jzts();
			window.location.href="<%=basePath%>linkagePullData/listAllByPage.do?linkageId="+linkageId;
		};
		
		//新增
		function add(linkageId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>linkagePullData/goAdd.do?parentId='+linkageId;
			 diag.Width = 500;
			 diag.Height =250;
			 diag.CancelEvent = function(){ //关闭事件
				 if('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display){
					 parent.location.href="<%=basePath%>linkagePullData/list.do?linkageId=${linkageId}&dnowPage=${page.currentPage}";
					/* $("#treeFrame", window.parent.document); */
					<%-- var src="<%=basePath%>linkagePullData/listAllByPage.do?linkageId=${'' == linkageId?'0':linkageId}&currentPage=${null == pd.dnowPage || '' == pd.dnowPage?'1':pd.dnowPage}";
					document.getElementById('treeFrame').src=src; --%>
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>linkagePullData/deleteLinkagePullData_new.do?linkageId="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>linkagePullData/goEdit.do?linkageId='+Id;
			 diag.Width = 500;
			 diag.Height =250;
			 diag.CancelEvent = function(){ //关闭事件
