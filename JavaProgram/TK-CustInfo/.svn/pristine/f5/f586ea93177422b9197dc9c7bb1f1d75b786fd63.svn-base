...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//导出
		function toExcel(){
			confirmExport=confirm("是否导出本页的数据？点击是导出本页的数据，点击否导出全部数据。");
			var isExportAll='all';
			if(confirmExport){
				isExportAll='no';
				$('#isExportAll').val(isExportAll);
			}
			$('#Form').attr("action",'questionStatic/exportExcel.do');
			$('#Form').submit();
			$('#isExportAll').val('');
			$('#Form').attr("action","questionStatic/goStatissticsData.do");
		}
