			...
			<div class="page-content">
				<div class="row">
					...
					<form action="department/${msg }.do" name="Form" id="Form" method="post">
						...
						<input type="hidden" name="PATH" id="PATH" value="${pds.PATH}"/>
						<c:if test="${pds.LEVE == null || pds.LEVE == ''}">
						<input type="hidden" name="LEVE" id="LEVE" value="1"/>
						</c:if>
						<c:if test="${pds.LEVE != null && pds.LEVE != ''}">
						<input type="hidden" name="LEVE" id="LEVE" value="${pds.LEVE}"/>
						</c:if>
						<div id="zhongxin">
						<table id="table_report" class="table table-striped table-bordered table-hover" style="margin-top:15px;">
							...
							<%-- <tr>
								...
								<td><input type="text" name="LEVE" id="LEVE" value="${pd.LEVE}" maxlength="32" placeholder="这里输入等级 " title="等级" style="width:76%;" /></td>
							</tr> --%>
							<tr>
