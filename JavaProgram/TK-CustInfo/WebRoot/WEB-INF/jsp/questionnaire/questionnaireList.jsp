	...
	<div class="row-fluid">
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class="center" style="width: 60px;">${var.USERNAME}</td>
								<td class="center" style="width: 60px;">${var.DEPARTMENT_NAME }</td>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<c:if test="${QX.copyQuestionnaire == 1 }">
						<a class="btn btn-small btn-success" onclick="copy();">复制</a>
					</c:if>
					<c:if test="${QX.tranSfer == 1 }">
						<a class="btn btn-small btn-success" onclick="zhuanyi();">转分配</a>
					</c:if>
					<c:if test="${QX.del == 1 }">
			...
			if(str==''){
				...
				if(ids.length>1){
					...
					return;
					 ...
					 diag.Drag=true;
					 diag.Title ="问卷复制";
					 diag.URL = '<%=basePath%>questionnaire/goCopyQuestionnaire.do?questionnaireId='+ids;
					 diag.Width = 850;
					 diag.Height = 310;
					 diag.CancelEvent = function(){ //关闭事件
						 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function zhuanyi(){
			...
			var diag = new top.Dialog();
		 	diag.Drag=true;
			diag.Title ="问卷转移";
			diag.URL = '<%=basePath%>questionnaire/goTransfer.do?questionnaireId='+str;
			diag.Width = 469;
			diag.Height = 200;
			diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
