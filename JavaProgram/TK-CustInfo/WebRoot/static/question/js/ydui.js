...
!function (window) {
    ...
    function CitySelect (element, options) {
        ...
        this.init(options.types,options.div_list_linkageId_id);
    }
    ...
    CitySelect.prototype.init = function (types,div_list_linkageId_id) {
        var _this = this,
        ...
        _this.createDOM(div_list_linkageId_id);

        _this.defaultSet = {
            provance: options.provance,
            city: options.city,
            area: options.area,
            levelfour:options.levelfour
        };
    };
    ...
    CitySelect.prototype.open = function () {
        ...
        if (defaultSet.city) {
        	var tagpam = _this.loadCity();
        	_this.backOffView(true);
            _this.setNavTxt(1, defaultSet.city,tagpam);
        }
        
        if (defaultSet.area) {
            var tagpam = _this.loadArea();
            _this.ForwardView(true);
            _this.setNavTxt(2, defaultSet.area, tagpam);
        }
        if (defaultSet.levelfour) {
        	var tagpam = _this.loadleve4();
            _this.ForwardViewTown(false);
            _this.setNavTxt(3, defaultSet.levelfour, tagpam);
        }
    ...
    CitySelect.prototype.createDOM = function (div_list_linkageId_id) {
        var _this = this;
        ...
        _this.$cityElement = $('' +
            ...
            '        <p class="cityselect-title">'+$("#"+div_list_linkageId_id).html()+'</p>' +
        ...
        _this.$cityElement.on('click.ydui.cityselect', '.cityselect-nav a', function () {
            ...
            if($this.index()==3){
            	_this.ForwardViewTown(true);
            }
            
        });
    };

    CitySelect.prototype.setNavTxt = function (index, txt, tagpam) {

        var $nav = this.$cityElement.find('.cityselect-nav a');
        //tagpam为-1，没有下级，无需请选择
        if(tagpam==-1 ){
        	$nav.eq(index).html(txt);
        	$nav.eq(index + 1).removeClass('crt').html('');
        	$nav.eq(index + 2).removeClass('crt').html('');
        	return;
        }
        if(index < 4 && $nav.removeClass('crt')){
			$nav.eq(index).html(txt);
			$nav.eq(index + 1).addClass('crt').html('请选择');
			$nav.eq(index + 2).removeClass('crt').html('');
		}else{
			$nav.eq(index).html(txt);
			$nav.eq(index + 2).addClass('crt').html('请选择');
    ...
    CitySelect.prototype.ForwardView = function (animate) {
    	this.$cityElement.find('.cityselect-content').removeClass('cityselect-nexttown');
    ...
    //向左移动100%
    CitySelect.prototype.ForwardViewTown = function (animate) {
        this.$cityElement.find('.cityselect-content').removeClass('cityselect-move-animate cityselect-prev')
        .addClass((animate ? 'cityselect-move-animate' : '') + ' cityselect-nexttown');
    };
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            var tag1 = $this.data('tagpam');
            _this.setNavTxt(tag, $this.text(), tag1);
            ...
            if(tag1 == -1){
                ...
                defaultSet.levelfour = $nav.eq(3).html();
                _this.returnValue();
                return;
            }
            if (tag == 0) {
    ...
    CitySelect.prototype.returnValue = function () {
        ...
        _this.$element.trigger($.Event('done.ydui.cityselect', {
            ...
            levelfour: defaultSet.levelfour
        }));
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        //a[]为空时，点击全部终止
        if (arr.length <= 0) {
        	arr.push($('<a class="' + ('全部' == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span>全部</span></a>').data({tag: 1,tagpam:-1}));
            tagpam=-1;
        }
        _this.fillItems(1, arr);
        var newArr = [];//清空下级数据
        _this.fillItems(3, newArr);
        _this.fillItems(2, newArr);
        return tagpam;
    };
  //三级点击
    CitySelect.prototype.loadArea = function () {
        ...
        $.each(areaData, function (k, v) {
        	if(v.t=='1'){
        		...
        		arr.push($('<a class="' + (v.n == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                	tag: 2,
                	tagpam:-1}));
        		if(v.n == _this.defaultSet.area){
        			tagpam=-1;
        		}
        		
        	}else{
        		//无下级数据
        		arr.push($('<a class="' + (v.n == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>' + v + '</span></a>').data({
                	tag: 2,
                	tagpam:-1}));
        		if(v.n == _this.defaultSet.area){
        			tagpam=-1;
        ...
        if (arr.length <= 0) {
            arr.push($('<a class="' + ('全部' == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>全部</span></a>').data({tag: 2,tagpam:-1}));
            tagpam=-1;
        }
    ...
    //四级点击
    CitySelect.prototype.loadleve4 = function () {
        var _this = this;
        var tagpam = 0;
        ...
        $.each(areaData, function (k, v) {
        	if(v.t=='1'){
        		//有下级数据
        		arr.push($('<a class="' + (v.n == _this.defaultSet.levelfour ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                	citys: v.a,
                	tag: 3}));
        	}else if(v.t=='0'){
        		//无下级数据
        		arr.push($('<a class="' + (v.n == _this.defaultSet.levelfour ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                	tag: 3,
                	tagpam:-1}));
        		if(v.n == _this.defaultSet.levelfour){
        			tagpam=-1;
        		}
        	}else{
        		//无下级数据
        		arr.push($('<a class="' + (v == _this.defaultSet.levelfour ? 'crt' : '') + '" href="javascript:;"><span>' + v + '</span></a>').data({
                	tag: 3,
                	tagpam:-1}));
        		if(v == _this.defaultSet.levelfour){
        			tagpam=-1;
        		}
        	}
        });
        if (arr.length <= 0) {
        	arr.push($('<a class="' + ('全部' == _this.defaultSet.levelfour ? 'crt' : '') + '" href="javascript:;"><span>全部</span></a>').data({
        		tag: 3,tagpam:-1}));
            tagpam=-1;
        }
