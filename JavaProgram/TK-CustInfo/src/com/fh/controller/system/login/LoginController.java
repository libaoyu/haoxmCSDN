...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").split(",fh,");//.replaceAll("qq313596790fh", "").replaceAll("QQ978336446fh", "")
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						user.getDepartment().setDEPARTMENT_ID(pd.getString("DEPARTMENT_ID"));
	...
	public ModelAndView defaultPage() throws Exception{
		ModelAndView mv = this.getModelAndView();
//		PageData pd = new PageData();
//		pd.put("userCount", Integer.parseInt(newuserService.getUserCount("").get("userCount").toString())-1);				//系统用户数
...
//		mv.addObject("pd",pd);
	...
	public void removeSession(String USERNAME){
		...
		session.removeAttribute(USERNAME + Const.SESSION_ROLE_RIGHTS);//角色权限
		session.removeAttribute(USERNAME + Const.SESSION_allmenuList);//全部菜单列表
		session.removeAttribute(USERNAME + Const.SESSION_menuList);//当前菜单
		session.removeAttribute(USERNAME + Const.SESSION_QX);//权限
		session.removeAttribute(Const.SESSION_userpds);
		session.removeAttribute(Const.SESSION_USERNAME);
		session.removeAttribute(Const.SESSION_USERROL);//用户对象
		session.removeAttribute("changeMenu");
		session.removeAttribute("DEPARTMENT_IDS");//部门
	...
	public static void main(String[] args) {
		String USERNAME="admin",PASSWORD="P@ssw3rd";
		String passwd = new SimpleHash("SHA-1", USERNAME, PASSWORD).toString();	//密码加密
		System.out.print(passwd);
	}
	// admin:fc4c004881fcb25892e0803cb7197b0655b13cd4
