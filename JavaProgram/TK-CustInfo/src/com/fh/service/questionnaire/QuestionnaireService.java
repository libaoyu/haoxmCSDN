...
//					boolean  flag= QRcode.encode(questionnaire.getQuestionUrl()+"?id="+pd.getString("questionnaireId")+"&level="+levels[i],Const.FILEPATHFILE+fileId+".jpg");
	...
	* 修改
	*/
//	public void edit1(Questionnaire questionnaire,PageData pd)throws Exception{
//		for (FileDetails detail:questionnaire.getFileDetails()) {
...
//		dao.delete("FileDetailsMapper.deleteByMasterId", pd);
		...
		String shortUri=pd.getString("shortUri");
		List<FileDetails> list=new ArrayList<FileDetails>();
		List<String> strQR = new ArrayList<String>();
		if(level!=null&&!level.isEmpty()){
			if(level.indexOf(",")>-1){
				String[] levels=level.split(",");
				for (int i = 0; i < levels.length&&isSuccess; i++) {
					//查询调查对象是否生成过二维码
					Map<String,String> map=ConversionUtil.getStaffLevelByParentId(questionnaire.getRespondentsParentId());
					String fileTitle = map.get(levels[i]);
					pd.put("fileTitle", fileTitle);
					PageData qrFile = (PageData) dao.findForObject("FileDetailsMapper.findQR", pd);
					if(qrFile!=null){ //二维码生成过
						strQR.add(qrFile.getString("fileId"));
					}else{
						String fileId=UuidUtil.get32UUID();
						String url=questionnaire.getQuestionUrl()+"?id="+pd.getString("questionnaireId")+"&level="+levels[i];
						// addby shanghz 生成短链接码+时间戳
						String shortUrl = ShortUrl.shortUrl(url+DateUtil.getSdfTimesSSS());
						boolean  flag= QRcode.encode(shortUri+shortUrl,Const.FILEPATHFILE+fileId+".jpg");
						if(flag){
							questionnaire.setRespondents(levels[i]);
							FileDetails detail=uploadImg(questionnaire,fileId,url,shortUrl);
							if(detail!=null){
								strQR.add(detail.getFileId());
								list.add(detail);
							}
						}else{
							isSuccess=false;
				...
				//查询调查对象是否生成过二维码
				Map<String,String> map=ConversionUtil.getStaffLevelByParentId(questionnaire.getRespondentsParentId());
				String fileTitle = map.get(level);
				pd.put("fileTitle", fileTitle);
				PageData qrFile = (PageData) dao.findForObject("FileDetailsMapper.findQR", pd);
				if(qrFile!=null){ //二维码生成过
					strQR.add(qrFile.getString("fileId"));
		...
		if( !isSuccess ){
			...
			if(strQR!=null && strQR.size()>0){
				pd.put("fileQRId", strQR);
				dao.delete("FileDetailsMapper.deleteByQR", pd);
			}
		}
	}
	...
	*/
	 ...
	 * @throws Exception
	 */
	public Questionnaire entityFindDataByIdForUpdate(PageData pd) throws Exception{
		return (Questionnaire)dao.findForObject("QuestionnaireMapper.entityFindDataByIdForUpdate", pd);
	}
			...
			for (Questions questions : oneList) {
				...
				for (Questions q : twoList) {
					if(questions.getQuestionId().equals(q.getParentId())){
						if(flag){
							q.setLevel(pd.getString("respondents"));
						}
				...
				if(flag){
					questions.setLevel(pd.getString("respondents"));
				}
...
//		}
	...
	public void updateQuestionnaireDepartment(PageData pd) throws Exception{
		...
		for (int i = 0; i < str.length; i++) {
			...
			pd1.put("questionnaireId",str[i]);
			pd1.put("USER_ID", pd.getString("USER_ID"));
			pd1.put("DEPARTMENT_ID", pd.getString("DEPARTMENT_ID"));
			list.add(pd1);
		}
		dao.batchUpdate("QuestionnaireMapper.updateQuestionnaireDepartment", list);
	}
}
